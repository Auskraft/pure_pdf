...
- Увеличен QR на экране поддержки: код занимает квадратную область и остаётся крупным на узких экранах.

## 1.8.3 — 2026

- Добавлена добровольная поддержка: сердечко в библиотеке, вход из настроек, экран с кнопкой оплаты, исходным QR и ссылками на условия и автора.
- Предложение поддержки появляется не раньше 14 дней и повторяется не чаще раза в 14 дней; после перехода к оплате больше не показывается автоматически. Окно оценки имеет приоритет.
